...
import tileworld.agent.Phase1Agent;
import tileworld.agent.Phase2Agent;
import tileworld.agent.Phase4Agent;
...
//    }
    ...
    public void start() {
        ...
        if (Parameters.useRandomWalkBaseline) {
        ...
        } else if (Parameters.usePhase4Agent) {
            int n = Math.max(1, Parameters.phase4AgentCount);
            for (int i = 1; i <= n; i++) {
                if (i > 1) {
                    pos = this.generateRandomLocation();
                }
                createAgent(new Phase4Agent("agent" + i, pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel, n));
            }
        } else if (Parameters.usePhase2Agent) {
            createAgent(new Phase2Agent("agent1", pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel));
            pos = this.generateRandomLocation();
            createAgent(new Phase2Agent("agent2", pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel));
        } else {
            createAgent(new Phase1Agent("agent1", pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel));
            pos = this.generateRandomLocation();
            createAgent(new Phase1Agent("agent2", pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel));
        }
